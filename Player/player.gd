...
func movement(): # checks what action is being pressed down
	...
	if mov != Vector2.ZERO:
		if walkTimer.is_stopped():
			if sprite.frame >= sprite.hframes - 1:
				...
				sprite.frame += 1
			walkTimer.start()
	# to move a CharacterBody2D object it is needed to set its velocity
	...
	move_and_slide() # this lets Godot know that we want the character to move -> automatically uses delta within its calculations
